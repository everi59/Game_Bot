...
async def lobby_page(callback: CallbackQuery,
    ...
    if len(lobby_stat_1) < 4:
        ...
        await callback.message.edit_text(text=f'Вы вошли в лобби №{callback_data.lobby_id}!\n\n'
                                              f'Участники:\n'
        ...
        await send_messages_to_users(bot=bot, message=f'{callback.from_user.full_name} зашел в лобби', users=lobby_stat_1)
    else:
...
async def exit_command(message: Message,
    ...
    await state.clear()
    await state.set_state(FSMLobbyClass.select_lobby)
    await users_without_lobbies_database.insert_users_message_id(chat_id=message.chat.id, message_id=bot_message.message_id)
    lobby_stat = lobby_database.get_lobby_stat(data['lobby'])
    for chat_id, message_id in people_without_lobby:
        try:
            await bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id,
...
async def ready_command_others(message: Message,
                               state: FSMContext):
    data = await state.get_data()
    people = await lobby_database.get_lobby_stat(data['lobby'])
    for i in people:
        if int(i.split('-')[0]) != message.chat.id:
            await bot.send_message(chat_id=int(i.split('-')[0]), text=f"""{message.from_user.full_name}: {message.text}""")
